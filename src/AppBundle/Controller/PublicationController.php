...
class PublicationController extends Controller{
  ...
  public function __construct() {
    ...
    public function indexAction(Request $request){
        ...
        if ($form->isSubmitted()) {
          if ($form->isValid()) {
            ...
            $status = 'La publicacion no se creo pq el formulario no es valido';
          }
    ...
    public function getPublications(Request $request){
      ...
      /*
      SELECT * FROM publications WHERE user_id = 8 OR user_id IN
      (SELECT followed FROM following WHERE user = 8);
      */
    ...
    public function removePublicationAction(Request $request, $id = null){
      ...
      if ($user->getId() == $publication->getUser()->getId()) {
        ...
        if ($flush == null) {
          ...
          $status = 'La publicación no pudo ser eliminada';
        }
      }else {
        $status = 'La publicación no te pertenece';
      }
